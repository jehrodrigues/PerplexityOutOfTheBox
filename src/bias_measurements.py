...
class BiasMeasurementCompiler:
    ...
    def compile(self):
        ...
        all_perplexity_path = os.path.join(save_folder, "all_perplexities.csv")
        ...
        for raw_result in tqdm(raw_results):
            ...
            noun = raw_result[0]["noun"]
            noun_gender = raw_result[0]["noun_gender"]
            noun_phrase = raw_result[0]["noun_phrase"]
            eval_labels = raw_result[0]["eval_labels"]
            ppl = raw_result[1]["metrics"]["ppl"]
            all_perplexities.extend([(axis, descriptor, template, noun, noun_gender, noun_phrase, eval_labels, ppl)])
        ...
        for axis in sorted(list(binned_perplexities.keys())):
            ...
            for template in tqdm(sorted(list(perplexities_for_axis.keys()))):
                ...
                # all perplexities
                """
                these_all_perplexities = [
                    (axis, descriptor, template, ppls)
                    for descriptor, ppls in perplexities_for_template.items()
                ]
                all_perplexities.extend(these_all_perplexities)
                """
        ...
        # saving all perplexities
        all_perplexity_df = pd.DataFrame(
            all_perplexities,
            columns=["axis", "descriptor", "template", "noun", "noun_gender", "noun_phrase", "eval_labels", "ppl"],
        )
        all_perplexity_df.to_csv(all_perplexity_path, index=False)
